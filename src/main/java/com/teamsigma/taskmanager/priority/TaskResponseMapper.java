...
/**
 * Task → 스코어링 ScoredTaskResponse 매핑. urgencyLevel(단일 진실)·reason(설명가능성)·stuckLevel(비수치심 정체등급)을 채운다.
 */
@Component
public class TaskResponseMapper {
    ...
    public ScoredTaskResponse toResponse(Task task, double score) {
        if (task == null) {
            return null;
        }
        double rounded = Math.round(score * 100.0) / 100.0;
        ScoredTaskResponse response = new ScoredTaskResponse(task, rounded);
